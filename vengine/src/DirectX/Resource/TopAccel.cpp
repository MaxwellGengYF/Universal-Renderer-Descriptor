...
void GetRayTransform(D3D12_RAYTRACING_INSTANCE_DESC& inst, float4x4 const& tr) {
	...
	*x = float4(tr._11, tr._12, tr._13, tr._14);
	float4* y = (float4*)(&inst.Transform[1][0]);
	*y = float4(tr._21, tr._22, tr._23, tr._24);
	float4* z = (float4*)(&inst.Transform[2][0]);
	*z = float4(tr._31, tr._32, tr._33, tr._34);
}
}// namespace detail
...
void TopAccel::Reserve(
	...
	auto RebuildBuffer = [&](
		...
		DefaultBuffer const* b = buffer
									 ? (DefaultBuffer const*)buffer
									 : (DefaultBuffer const*)nullptr;
	...
	if (RebuildBuffer(instBuffer, sizeof(D3D12_RAYTRACING_INSTANCE_DESC) * newCapacity, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE, true)) {
	...
	if (RebuildBuffer(accelBuffer, topLevelPrebuildInfo.ResultDataMaxSizeInBytes, D3D12_RESOURCE_STATE_RAYTRACING_ACCELERATION_STRUCTURE, false)) {
		...
		topLevelBuildDesc.Inputs.Flags =
			(D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAGS)(((uint)topLevelBuildDesc.Inputs.Flags)
