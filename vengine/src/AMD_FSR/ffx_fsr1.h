...
//
//
//                    AMD FidelityFX SUPER RESOLUTION [FSR 1] ::: SPATIAL SCALING & EXTRAS - v1.20210426
//
//
//------------------------------------------------------------------------------------------------------------------------------
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
// ABOUT
// =====
// This is part of the FidelityFX Super Resolution [FSR] umbrella.
...
//  [SRTM] Simple Reversible Tone-Mapper .......... HDR to {0 to 1} and back.
//  [TEPD] Temporal Energy Preserving Dither ...... Linear {0 to 1} to gamma 2.0 conversion.
...
//------------------------------------------------------------------------------------------------------------------------------
// The scalar uses a modified fast approximation to the standard lanczos(2) kernel.
// EASU runs in a single pass, so it applies an directionally and anisotropically adaptive radial lanczos.
...
//------------------------------------------------------------------------------------------------------------------------------
// Best to apply EASU in non-linear (aka perceptual, after conversion to sRGB).
// Directional adaption based on typical gaming anti-aliasing runs better in non-linear.
// Use 32-bit/pixel formats like 8:8:8:8 or 10:10:10:2.
//------------------------------------------------------------------------------------------------------------------------------
// Best to apply EASU at the end of the frame after tonemapping 
// but before {adding film grain, or composite of the UI}.
//------------------------------------------------------------------------------------------------------------------------------
// EASU scaling usage,
//  ...
//  // Include the portability header, example for Vulkan, with FP16 support.
//  // DX12 would be using A_HLSL_6_2 define instead of A_GLSL.
//  // To use FsrEasuCon() on the CPU, define A_CPU instead of A_GPU.
//  #define A_GLSL 1
//  #define A_GPU 1
//  #define A_HALF 1
...
//  #include "ffx_fsr1.h"
//  ...
//  // Vulkan example for the required callbacks.
//  // These are simple, just gather4 with the associated channel given normalized input.
//  // EASU uses gather4 to reduce position computation logic, and for free AoS-SoA conversion for FP16 path.
//  // The FP16 paths include output conversion back to packed 16-bit which the compiler should translate into D16 opcodes.
//  AF4 FsrEasuRF(AF2 p){return textureGather(sampler2D(tex,sam),p,0);}
//  AF4 FsrEasuGF(AF2 p){return textureGather(sampler2D(tex,sam),p,1);}
//  AF4 FsrEasuBF(AF2 p){return textureGather(sampler2D(tex,sam),p,2);}
//  AH4 FsrEasuRH(AF2 p){return AH4(textureGather(sampler2D(tex,sam),AF2(p),0));}
//  AH4 FsrEasuGH(AF2 p){return AH4(textureGather(sampler2D(tex,sam),AF2(p),1));}
//  AH4 FsrEasuBH(AF2 p){return AH4(textureGather(sampler2D(tex,sam),AF2(p),2));}
//  ...
//  // Setup function can be called from the CPU or GPU.
//  // This sets up constants for both FP32 and FP16 paths (they share the same constants).
...
//  ...
//  // Calling EASU scaling, a Vulkan example.
//  // Use a {64,1,1} workgroup, then use the workgroup reconfigure to get 8x8 tiles.
//  layout(local_size_x=64)in;
//  void main(){
//    // Remapped local xy in workgroup.
//    AU2 gxy=ARmp8x8(gl_LocalInvocationID.x)+AU2(gl_WorkGroupID.x<<3u,gl_WorkGroupID.y<<3u);
...
//    con2=cb.con2;
//    con3=cb.con3;
//    // Run the scalar, result goes in 'c'.
//    // The FP16 path is the same with 'AH4' as output and 'FsrEasuH()' as the function.
//    AF4 c;FsrEasuF(c.rgb,gxy,con0,con1,con2);
//    // Do any output processing after scaling, and store out color.
//==============================================================================================================================

//_____________________________________________________________/\_______________________________________________________________
...
//==============================================================================================================================
 ...
 // Prototypes, FP16's precision for coordinates is marginal, thus FP32 is used.
 //  +/- 2048 can be exactly represented, so there are ways to make FP16 work, but not in the general case.
 AH4 FsrEasuRH(AF2 p);
 AH4 FsrEasuGH(AF2 p);
 AH4 FsrEasuBH(AF2 p);
//------------------------------------------------------------------------------------------------------------------------------
...
// RCAS sharper does not support scaling, there is instead a much better scalar provided as a separate pass.
//------------------------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------------------------
// See the prior 'ffx_cas.h' header for more documentation on color-space and integration details.
// Only providing a very simple example below so it is easy to see the API changes from CAS.
// RCAS is very similar to CAS.
//------------------------------------------------------------------------------------------------------------------------------
// Usage example,
//  ...
//  // Include the portability header, example for Vulkan, with FP16 support.
//  // DX12 would be using A_HLSL_6_2 define instead of A_GLSL.
//  // To use FsrRcasCon() on the CPU, define A_CPU instead of A_GPU.
//  #define A_GLSL 1
//  #define A_GPU 1
//  #define A_HALF 1
//  // Include the 32-bit path.
//  #define FSR_RCAS_F 1
//  // Include the 16-bit paths.
//  #define FSR_RCAS_H 1
//  #define FSR_RCAS_HX2 1
//  #include "ffx_fsr1.h"
//  ...
//  // Vulkan example for the required callbacks.
//  AF4 FsrRcasLoadF(ASU2 p){return imageLoad(imgSrc,p);}
//  AH4 FsrRcasLoadH(ASW2 p){return AH4(imageLoad(imgSrc,ASU2(p)));}
//  AH4 FsrRcasLoadHx2(ASW2 p){return AH4(imageLoad(imgSrc,ASU2(p)));}
//  // Example for gamma 2.0 input (does gamma 2.0 to linear transform).
//  void FsrRcasInputF(inout AF1 r,inout AF1 g,inout AF1 b){r*=r;g*=g;b*=b;}
//  void FsrRcasInputH(inout AH1 r,inout AH1 g,inout AH1 b){r*=r;g*=g;b*=b;}
//  void FsrRcasInputHx2(inout AH2 r,inout AH2 g,inout AH2 b){r*=r;g*=g;b*=b;}
//  ...
//  // Setup function can be called from the CPU or GPU.
//  // This sets up constants for both FP32 and FP16 paths (they share the same constants).
...
//  AU4 con;
//  FsrRcasCon(con,
//   0.0); // The scale is {0.0 := maximum, to N>0, where N is the number of stops (halving) of the reduction of sharpness}.
//  ...
//  // Calling RCAS, a Vulkan example.
//  // Use a {64,1,1} workgroup, then use the workgroup reconfigure to get 8x8 tiles.
//  layout(local_size_x=64)in;
//  void main(){
//    // Remapped local xy in workgroup.
...
//    con=cb.con;
//    // Run the filter.
//    AF4 c;FsrRcasF(c.r,c.g,c.b,gxy,con);
// ---------------
...
//------------------------------------------------------------------------------------------------------------------------------
  ...
  #ifdef FSR_RCAS_DEBUG_CHECKER
   if((((ip.x^ip.y)>>8u)&1u)==0u){AF3 pix0=FsrRcasLoadF(ASU2(ip));
    pixR=pix0.r;pixG=pix0.g;pixB=pix0.b;FsrRcasInputF(pixR,pixG,pixB);return;}
  #endif 
//------------------------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------------------------
  // Debug a checker pattern of on/off tiles for visual inspection.
  #ifdef FSR_RCAS_DEBUG_CHECKER
   if((((ip.x^ip.y)>>8u)&1u)==0u){AH3 pix0=FsrRcasLoadH(ASU2(ip));
    pixR=pix0.r;pixG=pix0.g;pixB=pix0.b;FsrRcasInputH(pixR,pixG,pixB);return;}
  #endif 
//------------------------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------------------------
  // Debug a checker pattern of on/off tiles for visual inspection.
  #ifdef FSR_RCAS_DEBUG_CHECKER
   if((((ip.x^ip.y)>>8u)&1u)==0u){AH3 pix0=FsrRcasLoadHx2(ASW2(ip));AH3 pix1=FsrRcasLoadHx2(ASW2(ip)+ASW2(8,0));
    pixR=AH2(pix0.r,pix1.r);pixG=AH2(pix0.g,pix1.g);pixB=AH2(pix0.b,pix1.b);FsrRcasInputHx2(pixR,pixG,pixB);return;}
  #endif 
//------------------------------------------------------------------------------------------------------------------------------
...
//
//------------------------------------------------------------------------------------------------------------------------------
// This provides a way to take linear HDR color and convert it into a temporary {0 to 1} ranged post-tonemapped linear.
