...
class MD5_Impl {
public:
	/* Construct a MD5_Impl object with a vstd::string. */
	MD5_Impl(vstd::span<vbyte const> message, vbyte* digest);
	...
	void transform(const vbyte block[64]);

	/* Encodes input (usigned long) into output (vbyte). */
	void encode(const uint32_t* input, vbyte* output, size_t length);

	/* Decodes input (vbyte) into output (usigned long). */
	void decode(const vbyte* input, uint32_t* output, size_t length);

	/* Initialization the md5 object, processing another message block,
   * and updating the context.*/
	void init(const vbyte* input, size_t len);
...
const vbyte MD5_Impl::PADDING[64] = {0x80};
 ...
 *
 */
MD5_Impl::MD5_Impl(vstd::span<vbyte const> message, vbyte* digest)
	: digest(digest) {
 ...
 *
 */
void MD5_Impl::init(const vbyte* input, size_t len) {
 ...
 */
void MD5_Impl::transform(const vbyte block[64]) {
...
/**
* @Encodes input (unsigned long) into output (vbyte).
*
...
*
*/
void MD5_Impl::encode(const uint32_t* input, vbyte* output, size_t length) {

	for (size_t i = 0, j = 0; j < length; ++i, j += 4) {
		output[j] = (vbyte)(input[i] & 0xff);
		output[j + 1] = (vbyte)((input[i] >> 8) & 0xff);
		output[j + 2] = (vbyte)((input[i] >> 16) & 0xff);
		output[j + 3] = (vbyte)((input[i] >> 24) & 0xff);
	}
}
 ...
 *
 */
void MD5_Impl::decode(const vbyte* input, uint32_t* output, size_t length) {
	for (size_t i = 0, j = 0; j < length; ++i, j += 4) {
...
/**
 * @Convert digest to vstd::string value.
 *
 * @return the hex vstd::string of digest.
 *
 */
...
std::array<vbyte, MD5_SIZE> GetMD5FromString(vstd::string const& str) {
	using namespace detail;
	std::array<vbyte, MD5_SIZE> arr;
	MD5_Impl md5({reinterpret_cast<vbyte const*>(str.data()), str.size()}, arr.data());
	md5.GetDigest();
	return arr;
}
std::array<vbyte, MD5_SIZE> GetMD5FromArray(vstd::span<vbyte> data) {
	using namespace detail;
	std::array<vbyte, MD5_SIZE> arr;
...
MD5::MD5(vstd::string const& str)
	: MD5(vstd::span<vbyte const>(reinterpret_cast<vbyte const*>(str.data()), str.size())) {
}
MD5::MD5(vstd::string_view str)
	: MD5(vstd::span<vbyte const>(reinterpret_cast<vbyte const*>(const_cast<char const*>(str.data())), str.size())) {
}
MD5::MD5(vstd::span<vbyte const> bin) {
	using namespace detail;
	using namespace detail;
	MD5_Impl impl(bin, reinterpret_cast<vbyte*>(&data));
	impl.GetDigest();
}
...
VENGINE_UNITY_EXTERN void unity_get_md5(
	...
	uint64 dataLength,
	vbyte* destData) {
	detail::MD5_Impl md5(vstd::span<vbyte>(data, dataLength), destData);
	md5.GetDigest();
}
