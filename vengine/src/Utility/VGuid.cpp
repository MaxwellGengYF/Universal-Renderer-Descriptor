...
void ParseHex(vstd::string_view strv, Guid::GuidData& data) {
...
optional<Guid> Guid::TryParseGuid(vstd::string_view strv) {
	using namespace VGuid_Detail;
	GuidData data;
	switch (strv.size()) {
		case 22:
			StringUtil::DecodeFromBase64(strv, reinterpret_cast<vbyte*>(&data));
			return data;
		case 32:
			ParseHex(strv, data);
			return data;
...
Guid::Guid(vstd::string_view strv) {
	...
	switch (strv.size()) {
		...
		default:
			VEngine_Log("Wrong guid vstd::string length!\n");
			VENGINE_EXIT;
	}
}

Guid::Guid(vstd::span<vbyte> data) {
...
Guid::Guid(std::array<vbyte, sizeof(GuidData)> const& data) {
	memcpy(&this->data, data.data(), sizeof(GuidData));
}

vstd::string Guid::ToBase64() const {
	vstd::string result;
	StringUtil::EncodeToBase64({reinterpret_cast<vbyte const*>(&data), sizeof(data)}, result);
...
std::array<vbyte, sizeof(Guid::GuidData)> Guid::ToArray() const {
	std::array<vbyte, sizeof(GuidData)> arr;
...
VENGINE_UNITY_EXTERN void vguid_get_from_string(
	...
	int32 strLen,
	Guid* guidData) {
	*guidData = Guid(vstd::string_view(str, strLen)).ToBinary();
}
