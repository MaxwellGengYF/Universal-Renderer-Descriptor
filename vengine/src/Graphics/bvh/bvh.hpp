...
struct Bvh {
	...
	vstd::vector<size_t> primitive_indices;
	void Clear() {
		nodes.clear();
	...
	size_t node_count = 0;
	vstd::span<Node> GetNodes() {
		return vstd::span<Node>{nodes.data(), node_count};
	}
	size_t* GetPrimitiveIndices() {
		return primitive_indices.data();
	}
};
