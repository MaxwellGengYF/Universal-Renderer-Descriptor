...
#include <Graphics/bvh/bounding_box.hpp>
#include <Graphics/bvh/top_down_builder.hpp>
#include <Graphics/bvh/sah_based_algorithm.hpp>
#include <Graphics/bvh/radix_sort.hpp>
...
public:
	...
	vstd::vector<size_t> reference_data;
	vstd::vector<Scalar> cost_data;
	vstd::vector<Key> key_data;
	vstd::vector<Mark> mark_data;
	...
	void build(
		...
		reference_data.resize(primitive_count * 3);
		cost_data.resize(primitive_count * 3);
		key_data.resize(primitive_count * 2);
		mark_data.resize(primitive_count);

		std::array<Scalar*, 3> costs = {
			cost_data.data(),
			cost_data.data() + primitive_count,
			cost_data.data() + 2 * primitive_count};

		std::array<size_t*, 3> sorted_references;
		size_t* unsorted_references = bvh.primitive_indices.data();
		Key* sorted_keys = key_data.data();
		Key* unsorted_keys = key_data.data() + primitive_count;
...
#pragma omp single
				{
					...
					unsorted_references = reference_data.data() + axis * primitive_count;
					// Make sure that one array is the final array of references used by the BVH
					if (axis != 0 && sorted_references[axis] == bvh.primitive_indices.data())
						std::swap(sorted_references[axis], unsorted_references);
					assert(axis < 2 || sorted_references[0] == bvh.primitive_indices.data() || sorted_references[1] == bvh.primitive_indices.data());
				}

#pragma omp for
				for (int64_t i = 0; i < primitive_count; ++i) {
...
#pragma omp single
			{
				BuildTask first_task(*this, bboxes, centers, sorted_references, costs, mark_data.data());
...
public:
	...
	SweepSahBuildTask(
		...
		Mark* marks)
		: builder(builder), bboxes(bboxes), centers(centers), references{references[0], references[1], references[2]}, costs{costs[0], costs[1], costs[2]}, marks(marks) {}
