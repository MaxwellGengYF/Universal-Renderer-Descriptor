...
#include <Graphics/bvh/utilities.hpp>
...
#pragma omp single
		{
			...
			if (per_thread_data_size < data_size) {
				per_thread_buckets.clear();
				per_thread_buckets.resize(data_size);
...
#pragma omp for
			for (int64 i = 0; i < bucket_count; i++) {
...
#pragma omp single
	...
	static typename SizedIntegerType<sizeof(T) * CHAR_BIT>::Unsigned make_key(T x) {
		...
		using S = typename SizedIntegerType<sizeof(T) * CHAR_BIT>::Signed;
		auto mask = U(1) << (sizeof(T) * CHAR_BIT - 1);
		auto y = as<U>(x);
		return (y & mask ? (-(S)y) ^ mask : y) ^ mask;
	}

private:
	vstd::vector<size_t> per_thread_buckets;
