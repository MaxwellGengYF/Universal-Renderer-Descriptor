...
void TestNoSql(toolhub::db::Database* dbBase, toolhub::db::CompareFlag flag) {
	...
	auto db = vstd::create_unique(dbBase->CreateSimpleNoSql(""));
	...
	auto serde = vstd::create_unique(dbBase->CreateDatabase());
	...
	//Find One with kv vec
		...
		if constexpr (Delete) {
			auto kv = db->FindOneAndDelete_And(vstd::get_rvalue_ptr(vstd::GetSpanIEnumerable_Move(vstd::span<INoSqlDatabase::CompareKey>(kvs))));
			PrintKV(kv);
		} else {

			auto kv = db->FindOne_And(vstd::get_rvalue_ptr(vstd::GetSpanIEnumerable_Move(vstd::span<INoSqlDatabase::CompareKey>(kvs))));
			PrintKV(kv);
		}
	}
	//Find All with kv vec
		...
		kvs.emplace_back(99999, nullptr, flag);
		if constexpr (Delete) {
			auto ite = db->FindAllAndDelete_And(vstd::get_rvalue_ptr(vstd::GetSpanIEnumerable_Move(vstd::span<INoSqlDatabase::CompareKey>(kvs))));
			PrintAll(ite);
		} else {
			auto ite = db->FindAll_And(vstd::get_rvalue_ptr(vstd::GetSpanIEnumerable_Move(vstd::span<INoSqlDatabase::CompareKey>(kvs))));
			PrintAll(ite);
		}
	}
	if constexpr (i != 0) {
