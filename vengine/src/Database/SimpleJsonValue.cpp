...
void SimpleJsonValueDict::PrintYaml(vstd::string& str, size_t space) const {
	if (vars.Find(int64(0))) {
		...
		size_t sz = 0;
		for (auto&& kv : vars) {
			if (!kv.first.value.template IsTypeOf<vstd::string>()) {
				continue;
			}
...
void SimpleJsonValueDict::PrintYaml(vstd::string& str, SimpleJsonKey::ValueType const& key, size_t space) const {
	if (key.template IsTypeOf<vstd::string>()) {
		if (!str.empty() && *(str.end() - 1) != '\n') str << '\n';
		auto&& keyStr = key.template force_get<vstd::string>();
		auto idx = StringUtil::GetFirstIndexOf(keyStr, '|');
		if (idx == -1) {
			str << keyStr << ": ";
		} else {
			str << "--- "
				<< vstd::string_view(keyStr.data(), idx)
				<< "\n"
				<< vstd::string_view(keyStr.data() + idx + 1, keyStr.data() + keyStr.size())
				<< ": ";
		}
