...
public:
	...
	using FieldPool = vstd::Pool<Field, VEngine_AllocType::VEngine, false>;
...
private:
	...
	bool SetLastFieldState(vstd::string& errorStr) {
		if (fieldStack.empty()) {
			...
			if (lastField->data.GetType() == 1) {
				...
				(lastField->data.template get<1>().end() - 1)->second = curField;
			} else {
				streamer = StreamState::SearchContinue;
				lastField->data.template get<2>().emplace_back(curField);
			}
		}
		return true;
	}
	...
	bool SetStringField(T&& str, vstd::string& errorStr) {
		switch (streamer) {
			...
			case StreamState::SearchKey: {
				...
				lastField->data.template get<1>().emplace_back(std::forward<T>(str), nullptr);
				streamer = StreamState::SearchKeyValueCut;
			} break;
			case StreamState::SearchElement:
				if (lastField->data.GetType() == 1) {
					Field* field = fieldPool->New(std::forward<T>(str));
					(lastField->data.template get<1>().end() - 1)->second = field;
					streamer = StreamState::SearchContinue;
				} else {
					Field* field = fieldPool->New(std::forward<T>(str));
					lastField->data.template get<2>().emplace_back(field);
					streamer = StreamState::SearchContinue;
				}
				break;
			default:
	...
	bool M_Parse(vstd::string_view str, vstd::string& errorStr) {
	...
	WriteJsonVariant PrintField(Field* field) {
		switch (field->data.GetType()) {
			case 0: {
				auto&& v = field->data.template get<0>();
				return WriteJsonVariant(vstd::string_view(v.begin(), v.end()));
			}
			case 1: {
				auto&& v = field->data.template get<1>();
				return WriteJsonVariant(PrintDict(v));
			}
			case 2: {
				auto&& v = field->data.template get<2>();
				return WriteJsonVariant(PrintArray(v));
			}
			case 3: {
				auto&& v = field->data.template get<3>();
				return WriteJsonVariant(v);
			}
			case 4: {
				auto&& v = field->data.template get<4>();
				return WriteJsonVariant(v);
			}
			case 5: {
				auto&& v = field->data.template get<5>();
				return WriteJsonVariant(v);
			}
			case 6: {
				auto&& v = field->data.template get<6>();
				return WriteJsonVariant(v);
			}
			default:
		...
		for (auto&& i : v) {
			auto key = [&]() {
				switch (i.first.GetType()) {
					case 0: {
						auto&& v = i.first.template get<0>();
						return Key(vstd::string_view(v.begin(), v.size()));
					}
					case 1: {
						auto&& v = i.first.template get<1>();
						return Key(v);
					}
					case 2: {
						auto&& v = i.first.template get<2>();
						return Key(v);
					}
					default:
						return Key();
	...
	vstd::string_view GetKeyword(char const*& ptr, char const* const end) {
		char const* start = ptr;
		if constexpr (!escapeFirst) {
			if (recorders->outsideStates[*ptr] != OutsideState::Keyword) {
				return vstd::string_view(start, start);
			}
		}
		ptr++;
		while (ptr != end) {
		...
		switch (streamer) {
			...
			case StreamState::SearchElement: {
				...
				switch (value.data.GetType()) {
					case 0://string
						if (!SetStringField(value.data.template get<0>(), errorStr)) return false;
						break;
					case 1://dict
					{
						if (lastField->data.GetType() == 1) {
							streamer = StreamState::SearchContinue;
							(lastField->data.template get<1>().end() - 1)->second = fieldPool->New(value.data.template get<1>());
						} else {
							streamer = StreamState::SearchContinue;
							lastField->data.template get<2>().emplace_back(fieldPool->New(value.data.template get<1>()));
						}
					} break;
					case 2://array
						if (lastField->data.GetType() == 1) {
							streamer = StreamState::SearchContinue;
							(lastField->data.template get<1>().end() - 1)->second = fieldPool->New(value.data.template get<2>());
						} else {
							streamer = StreamState::SearchContinue;
							lastField->data.template get<2>().emplace_back(fieldPool->New(value.data.template get<2>()));
						}
						break;
					case 3://int
						if (!SetStringField(value.data.template get<3>(), errorStr)) return false;
						break;
					case 4://double
						if (!SetStringField(value.data.template get<4>(), errorStr)) return false;
						break;
					case 5://guid
						if (!SetStringField(value.data.template get<5>(), errorStr)) return false;
						break;
					case 6:
						if (!SetStringField(value.data.template get<6>(), errorStr)) return false;
						break;
					case 7:
						if (!SetStringField(value.data.template get<7>(), errorStr)) return false;
						break;
				}
				//SetStringField<
			} break;
...
bool SimpleJsonParser::BeginPath(char const*& ptr, char const* const end, vstd::string& errorStr) {
	...
	vstd::string_view pathName = vstd::string_view(start, ptr);
	++ptr;
	auto GenerateSub = [&]() -> bool {
		...
		subParser = &subParsers->emplace_back(SimpleJsonParser(keywords, fieldPool, subParsers));
		if (!subParser->M_Parse(vstd::string_view(reinterpret_cast<char const*>(fileVec.data()), fileVec.size()), errorStr)) {
			return false;
		}
	...
	switch (streamer) {
		...
		case StreamState::SearchElement:
			...
			if (lastField) {
				if (lastField->data.GetType() == 1) {
					streamer = StreamState::SearchContinue;
					(lastField->data.template get<1>().end() - 1)->second = subParser->rootField;
				} else {
					streamer = StreamState::SearchContinue;
					lastField->data.template get<2>().emplace_back(subParser->rootField);
				}
			} else {
...
vstd::optional<ParsingException> RunParse(
	T* ptr,
	IJsonDatabase* db,
	vstd::string_view str, bool clearLast) {
	if (clearLast) {
		ptr->Reset();
	}
