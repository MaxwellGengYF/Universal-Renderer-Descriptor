...
requires(!(std::is_const_v<T> || std::is_reference_v<T>)) class vector {
public:
	using value_type = T;
	using reference = T&;
	using pointer = T*;
...
public:
	...
	requires(detail::AllFunction<Func...>()) void push_back_func(size_t count, Func&&... f) {
		ResizeRange(count);
		auto endPtr = vec.arr + mSize;
		size_t index = 0;
	...
	template<typename Func>
	requires(std::is_invocable_v<Func, T&>) void compact(Func&& f) {
		T* curPtr = begin();
		T* lastPtr;
		T* ed = end();
		while (curPtr != ed) {
	...
	void operator=(span<T> list) {
		clear();
		reserve(list.size());
		mSize = list.size();
		auto ptr = &static_cast<T&>(*list.data());
		if constexpr (!(std::is_trivially_copy_constructible_v<T>)) {
			for (size_t i = 0; i < mSize; ++i) {
				new (vec.arr + i) T(ptr[i]);
			}
	...
	void operator=(span<T const> list) {
		clear();
		reserve(list.size());
		mSize = list.size();
		auto ptr = &static_cast<T const&>(*list.data());
		if constexpr (!(std::is_trivially_copy_constructible_v<T>)) {
			for (size_t i = 0; i < mSize; ++i) {
				new (vec.arr + i) T(ptr[i]);
			}
