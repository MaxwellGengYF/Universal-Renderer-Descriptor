...
private:
	inline static const vbyte bitOffsetArray[8]{
		0b00000001,
		0b00000010,
		0b00000100,
		0b00001000,
		...
		0b01000000,
		0b10000000};
	inline static const vbyte bitOffsetReversedArray[8]{
		0b11111110,
		0b11111101,
		0b11111011,
		0b11110111,
...
public:
	...
	inline BitArray(size_t length) : length(length) {
		...
		ptr = new vbyte[capa];
		memset(ptr, 0, sizeof(vbyte) * capa);
	}
	inline void Reset(bool target) {
		size_t capa = (length % 8 > 0) ? length / 8 + 1 : length / 8;
		memset(ptr, target ? 255 : 0, sizeof(vbyte) * capa);
	}
	inline ~BitArray() {
		if (ptr) {
			delete ptr;
...
private:
	...
	vbyte* ptr() const {
		return reinterpret_cast<vbyte*>(&data);
	}
	bool Get(size_t index) const {
#ifdef DEBUG
		if (index >= length) throw "Index Out of Range!";
