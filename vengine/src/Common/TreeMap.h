...
private:
	...
	template<typename Key>
	std::pair<NodePtr, int32> searchClosestTreeHelper(NodePtr node, Key const& key) const {
		auto compResult = comp(node->data.first, key);
		static_assert(std::is_same_v<decltype(compResult), int32_t>, "compare result must be int32");
		if (compResult == 0) {
			return {node, 0};
		...
		if (compResult > 0) {
			...
			return searchClosestTreeHelper(node->left, key);
		}
		if (!node->right) {
			return {node, -1};
		}
		return searchClosestTreeHelper(node->right, key);
	}
	template <typename Key>
	std::pair<NodePtr, int32> searchGEqual(NodePtr node, Key const& key) const {
		auto compResult = comp(node->data.first, key);
		static_assert(std::is_same_v<decltype(compResult), int32_t>, "compare result must be int32");
		if (compResult == 0) {
			return {node, 0};
		...
		if (compResult > 0) {
			if(!node->left) return {node, 1};
			auto lp = searchGEqual(node->left, key);
			if (lp.second >= 0) {
				return lp;
			}
			return {node, compResult};
		}
		if(!node->right) return {node, compResult};
		auto rp = searchGEqual(node->right, key);
		if(rp.second >= 0)
			return rp;
		return {node, compResult};
	}
	template <typename Key>
	std::pair<NodePtr, int32> searchGreater(NodePtr node, Key const& key) const {
		auto compResult = comp(node->data.first, key);
		static_assert(std::is_same_v<decltype(compResult), int32_t>, "compare result must be int32");
		if (compResult > 0) {
			if(!node->left) return {node, compResult};
			auto lp = searchGreater(node->left, key);
			if (lp.second > 0) {
				return lp;
			}
			return {node, compResult};
		...
		if(!node->right) return {node, compResult};
		auto rp = searchGreater(node->right, key);
		if(rp.second > 0)
			return rp;
		return {node, compResult};
	}
	template <typename Key>
	std::pair<NodePtr, int32> searchLEqual(NodePtr node, Key const& key) const {
		auto compResult = comp(node->data.first, key);
		static_assert(std::is_same_v<decltype(compResult), int32_t>, "compare result must be int32");
		if (compResult == 0) {
			return {node, 0};
		...
		if (compResult < 0) {
			if(!node->right) return {node, -1};
			auto lp = searchLEqual(node->right, key);
			if (lp.second <= 0) {
				return lp;
			}
			return {node, compResult};
		...
		if(!node->left) return {node, compResult};
		auto rp = searchLEqual(node->left, key);
		if(rp.second <= 0)
			return rp;
		return {node, compResult};
	}
	template <typename Key>
	std::pair<NodePtr, int32> searchLess(NodePtr node, Key const& key) const {
		auto compResult = comp(node->data.first, key);
		static_assert(std::is_same_v<decltype(compResult), int32_t>, "compare result must be int32");
		if (compResult < 0) {
			if(!node->right) return {node, compResult};
			auto lp = searchLess(node->right, key);
			if (lp.second < 0) {
				return lp;
			}
			return {node, compResult};
		...
		if(!node->left) return {node, compResult};
		auto rp = searchLess(node->left, key);
		if(rp.second < 0)
			return rp;
		return {node, compResult};
	}
	...
	requires(std::is_invocable_v<Func, Element const&>)
...
public:
	...
	Iterator find(Key const& k) const {
		if (!this->root) return {nullptr, this};
	...
	std::pair<Iterator, int32> find_closest(Key const& k) {
		if(!this->root) return {Iterator{nullptr, 0}, 0};
		auto pair = searchClosestTreeHelper(this->root, k);
		return {Iterator{pair.first, this}, pair.second};
	}

	template<typename Key>
	Iterator find_gequal(Key const& k) {
		if(!this->root) return Iterator{nullptr, this};
		auto pair = searchGEqual(this->root, k);
		if(pair.second < 0) return Iterator{nullptr, this};
		return Iterator{pair.first, this};
	}
	template<typename Key>
	Iterator find_lequal(Key const& k) {
		if(!this->root) return Iterator{nullptr, this};
		auto pair = searchLEqual(this->root, k);
		if(pair.second > 0) return Iterator{nullptr, this};
		return Iterator{pair.first, this};
	}
	template<typename Key>
	Iterator find_greater(Key const& k) {
		if(!this->root) return Iterator{nullptr, this};
		auto pair = searchGreater(this->root, k);
		if(pair.second < 0) return Iterator{nullptr, this};
		return Iterator{pair.first, this};
	}
	template<typename Key>
	Iterator find_less(Key const& k) {
		if(!this->root) return Iterator{nullptr, this};
		auto pair = searchLess(this->root, k);
		if(pair.second > 0) return Iterator{nullptr, this};
		return Iterator{pair.first, this};
	}
	...
	requires(std::is_constructible_v<K, Key&&> && detail::MapConstructible<V, Value&&...>::value)
