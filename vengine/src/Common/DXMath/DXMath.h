...
template<size_t len>
static size_t Int32ArrayHash(const uint32_t* ptr) {
	constexpr size_t byteSize = len * sizeof(uint32_t);
	if constexpr (byteSize < (sizeof(size_t) / 2)) {
		return vstd_xxhash_gethash_small(ptr, byteSize);
	} else {
		return vstd_xxhash_gethash(ptr, byteSize);
	}
}
}// namespace vec
template<>
struct hash<uint2> {
	inline uint64_t operator()(uint2 const& value) const noexcept {
		...
		return vec_detail::Int32ArrayHash<2>(ptr);
	}
};
template<>
struct hash<uint3> {
	inline uint64_t operator()(uint3 const& value) const noexcept {
		uint32_t const* ptr = (uint32_t const*)&value;
		return vec_detail::Int32ArrayHash<3>(ptr);
	}
};
template<>
struct hash<uint4> {
	inline uint64_t operator()(uint4 const& value) const noexcept {
		uint32_t const* ptr = (uint32_t const*)&value;
		return vec_detail::Int32ArrayHash<4>(ptr);
	}
};
template<>
struct hash<int2> {
