...
*/
inline vstd::vector<cudaGraphNode_t> cuda_get_graph_nodes(cudaGraph_t graph) {
  size_t num_nodes = cuda_get_graph_num_nodes(graph);
  vstd::vector<cudaGraphNode_t> nodes(num_nodes);
  TF_CHECK_CUDA(
...
*/
inline vstd::vector<cudaGraphNode_t> cuda_get_graph_root_nodes(cudaGraph_t graph) {
...
*/
inline vstd::vector<std::pair<cudaGraphNode_t, cudaGraphNode_t>>
cuda_get_graph_edges(cudaGraph_t graph) {
  size_t num_edges = cuda_get_graph_num_edges(graph);
  vstd::vector<cudaGraphNode_t> froms(num_edges), tos(num_edges);
  TF_CHECK_CUDA(
    cudaGraphGetEdges(graph, froms.data(), tos.data(), &num_edges),
    "failed to get native graph edges"
  );
  vstd::vector<std::pair<cudaGraphNode_t, cudaGraphNode_t>> edges(num_edges);
...
class cudaGraph : public CustomGraphBase {
  ...
  private:
    ...
    vstd::vector<std::unique_ptr<cudaNode>> _nodes;
    //vstd::vector<cudaNode*> _nodes;
};
...
class cudaNode {
  ...
  struct Capture {
    ...
    vstd::function<void(cudaStream_t)> work;
