...
when declaring standard library containers (e.g. vstd::vector).
*/
template<typename T>
class cudaDeviceAllocator {
...
//template <typename T>
//using cudaDeviceVector = vstd::vector<NoInit<T>, cudaDeviceAllocator<NoInit<T>>>;

//template <typename T>
//using cudaUSMVector = vstd::vector<T, cudaUSMAllocator<T>>;
