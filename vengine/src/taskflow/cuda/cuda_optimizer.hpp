...
class cudaCapturingBase {
  ...
  protected:
    ...
    vstd::vector<cudaNode*> _toposort(cudaGraph&);
    vstd::vector<vstd::vector<cudaNode*>> _levelize(cudaGraph&);
};

// Function: _toposort
inline vstd::vector<cudaNode*> cudaCapturingBase::_toposort(cudaGraph& graph) {

  vstd::vector<cudaNode*> res;
...
// Function: _levelize
inline vstd::vector<vstd::vector<cudaNode*>>
cudaCapturingBase::_levelize(cudaGraph& graph) {
  ...
  vstd::vector<vstd::vector<cudaNode*>> level_graph(max_level+1);
  for(auto& u : graph._nodes) {
    auto hu = std::get_if<cudaNode::Capture>(&u->_handle);
...
class cudaRoundRobinCapturing : public cudaCapturingBase {
  ...
  private:
    ...
    void _reset(vstd::vector<vstd::vector<cudaNode*>>& graph);
...
inline void cudaRoundRobinCapturing::_reset(
  vstd::vector<vstd::vector<cudaNode*>>& graph
) {
  //level == global id
