...
class ObjectPool {
	...
	vstd::Pool<Block, VEngine_AllocType::VEngine, true> blockPool;
	vstd::spin_mutex mtx;
	Block* alloc_block() {
		return blockPool.New_Lock(mtx);
	}
	void dealloc_block(Block* b) {
		blockPool.Delete_Lock(mtx, b);
	}
...
// Constructor
template<typename T, size_t S>
ObjectPool<T, S>::ObjectPool(unsigned t) ://_heap_mask   {(_next_pow2(t) << 1) - 1u},
...
template<class P, class Q>
constexpr P* ObjectPool<T, S>::_parent_class_of(
	const Q* ptr, const Q P::*member) const {
	return (P*)((char*)ptr - _offset_in_class(member));
}
