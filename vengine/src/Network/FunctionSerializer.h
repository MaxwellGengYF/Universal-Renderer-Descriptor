...
void ReverseBytes(T& num) {
	...
	if constexpr (sizeof(Type) == 2) {
		vbyte const* p = reinterpret_cast<vbyte const*>(&num);
		uint16_t result = 0;
		vbyte ofst = 8;
		for (vbyte i = 0; i < 2; ++i) {
...
struct SerDe {
	...
	static T Get(vstd::span<vbyte const>& data) {
		using Type = std::remove_cvref_t<T>;
		if (reverseBytes) {
			Type ptr = *reinterpret_cast<Type const*>(data.data());
			ReverseBytes(ptr);
			data = vstd::span<vbyte const>(data.data() + sizeof(Type), data.size() - sizeof(Type));
			return ptr;
		} else {
	...
	static void Set(T const& data, vector<vbyte>& vec) {
		using Type = std::remove_cvref_t<T>;
		if (reverseBytes) {
			Type tempData = data;
			ReverseBytes(tempData);
			vec.push_back_all(reinterpret_cast<vbyte const*>(&tempData), sizeof(Type));
		} else {
			vec.push_back_all(reinterpret_cast<vbyte const*>(&data), sizeof(Type));
		}
	}
};
template<bool reverseBytes>
struct SerDe<vstd::string, reverseBytes> {
	static vstd::string Get(vstd::span<vbyte const>& sp) {
		auto strLen = SerDe<uint, reverseBytes>::Get(sp);
		auto ptr = sp.data();
		sp = vstd::span<vbyte const>(ptr + strLen, sp.size() - strLen);
		return vstd::string(reinterpret_cast<char const*>(ptr), strLen);
	}
	static void Set(vstd::string const& data, vector<vbyte>& arr) {
		SerDe<uint, reverseBytes>::Set(data.size(), arr);
		arr.push_back_all(reinterpret_cast<vbyte const*>(data.data()), data.size());
	}
};
template<bool reverseBytes>
struct SerDe<vstd::string_view, reverseBytes> {
	static vstd::string_view Get(vstd::span<vbyte const>& sp) {
		auto strLen = SerDe<uint, reverseBytes>::Get(sp);
		auto ptr = sp.data();
		sp = vstd::span<vbyte const>(ptr + strLen, sp.size() - strLen);
	...
	static void Set(vstd::string_view const& data, vector<vbyte>& arr) {
		SerDe<uint, reverseBytes>::Set(data.size(), arr);
		arr.push_back_all(reinterpret_cast<vbyte const*>(data.begin()), data.size());
	}
};
...
struct SerDe<variant<Args...>, reverseBytes> {
	...
	template<typename T>
	static void ExecuteGet(void* placePtr, vstd::span<vbyte const>& sp) {
		new (placePtr) Value(SerDe<T, reverseBytes>::Get(sp));
	}
	template<typename T>
	static void ExecuteSet(void const* placePtr, vector<vbyte>& sp) {
		SerDe<T, reverseBytes>::Set(*reinterpret_cast<T const*>(placePtr), sp);
	}
	static Value Get(vstd::span<vbyte const>& sp) {
		auto type = SerDe<vbyte, reverseBytes>::Get(sp);
		funcPtr_t<void(void*, vstd::span<vbyte const>&)> ptrs[sizeof...(Args)] = {
			ExecuteGet<Args>...};
		Value v;
	...
	static void Set(Value const& data, vector<vbyte>& arr) {
		SerDe<vbyte, reverseBytes>::Set(data.GetType(), arr);
		funcPtr_t<void(void const*, vector<vbyte>&)> ptrs[sizeof...(Args)] = {
			ExecuteSet<Args>...};
...
template<bool reverseBytes>
struct SerDe<vstd::span<vbyte const>, reverseBytes> {
	using Value = vstd::span<vbyte const>;
...
struct SerDeAll_Impl<Ret(Args...)> {
	...
	static Ret CallMemberFunc(Class* ptr, Func func, vstd::span<vbyte const> data) {
	...
	static decltype(auto) Call(
		Func&& func) {
		return [f = std::forward<Func>(func)](vstd::span<vbyte const> data) {
