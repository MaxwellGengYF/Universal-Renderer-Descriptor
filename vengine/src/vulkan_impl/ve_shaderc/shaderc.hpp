...
public:
	...
	vstd::string GetErrorMessage() const {
		if (!compilation_result_) {
			return "";
		}
...
public:
	...
	void AddMacroDefinition(const vstd::string& name) {
		AddMacroDefinition(name.c_str(), name.size(), nullptr, 0u);
	}

	// Adds a predefined macro to the compilation options.
	void AddMacroDefinition(const vstd::string& name, const vstd::string& value) {
	...
	void SetIncluder(vstd::unique_ptr<IncluderInterface>&& includer) {
	...
	void SetHlslRegisterSetAndBindingForStage(shaderc_shader_kind shader_kind,
											  const vstd::string& reg,
											  const vstd::string& set,
											  const vstd::string& binding) {
	...
	// Copies the parameter strings.
	void SetHlslRegisterSetAndBinding(const vstd::string& reg,
									  const vstd::string& set,
									  const vstd::string& binding) {
...
private:
	...
	vstd::unique_ptr<IncluderInterface> includer_;
...
public:
	...
	// a vstd::string, and we assume the entry point is "main".
	SpvCompilationResult CompileGlslToSpv(const vstd::string& source_text,
										  shaderc_shader_kind shader_kind,
										  const char* input_file_name,
										  const CompileOptions& options) const {
	...
	// vstd::string.
	SpvCompilationResult AssembleToSpv(const vstd::string& source_assembly,
									   const CompileOptions& options) const {
	...
	// compilation result.
	// Like the first AssembleToSpv method but the source is provided as a
	// vstd::string and also uses default compiler options.
	SpvCompilationResult AssembleToSpv(const vstd::string& source_assembly) const {
		return SpvCompilationResult(shaderc_assemble_into_spv(
	...
	// is provided as a vstd::string.  Options are otherwise similar to
	// the first CompileToSpv method.
	AssemblyCompilationResult CompileGlslToSpvAssembly(
		const vstd::string& source_text, shaderc_shader_kind shader_kind,
	...
	// PreprocessGlsl method but the source is provided as a vstd::string.
