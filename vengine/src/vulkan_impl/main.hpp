...
void HelloTriangleApplication::setupDebugMessenger() {
	...
	auto value = CreateDebugUtilsMessengerEXT(instance, &createInfo, nullptr, &debugMessenger);
	if (value != VK_SUCCESS) {
		VEngine_Log("failed to set up debug messenger!");
		VENGINE_EXIT;
	}
}
...
HelloTriangleApplication::QueueFamilyIndices HelloTriangleApplication::findQueueFamilies(VkPhysicalDevice device) {
	...
	vstd::vector<VkQueueFamilyProperties> queueFamilies(queueFamilyCount);
...
bool HelloTriangleApplication::checkDeviceExtensionSupport(VkPhysicalDevice device) {
	...
	vstd::HashMap<vstd::string> requiredExtensions(deviceExtensions.size());
...
void HelloTriangleApplication::pickPhysicalDevice() {
	...
	if (deviceCount == 0) {
		VEngine_Log("failed to find GPUs with Vulkan support!");
		VENGINE_EXIT;
	}
	vstd::vector<VkPhysicalDevice> devices(deviceCount);
	...
	if (physicalDevice == VK_NULL_HANDLE) {
		VEngine_Log("failed to find a suitable GPU!");
		VENGINE_EXIT;
	}
	indices = findQueueFamilies(physicalDevice);
}
VkSurfaceFormatKHR HelloTriangleApplication::chooseSwapSurfaceFormat(const vstd::vector<VkSurfaceFormatKHR>& availableFormats) {
...
VkPresentModeKHR HelloTriangleApplication::chooseSwapPresentMode(const vstd::vector<VkPresentModeKHR>& availablePresentModes) {
...
void HelloTriangleApplication::createLogicalDevice() {
	...
	if (vkCreateDevice(physicalDevice, &createInfo, nullptr, &device) != VK_SUCCESS) {
		VEngine_Log("failed to create logical device!");
		VENGINE_EXIT;
	}
...
void HelloTriangleApplication::createSurface() {
	...
	if (vkCreateWin32SurfaceKHR(instance, &createInfo, nullptr, &surface) != VK_SUCCESS) {
		VEngine_Log("failed to create window surface!");
		VENGINE_EXIT;
	}
}
...
HelloTriangleApplication::HelloTriangleApplication()
	...
	window = glfwCreateWindow(WIDTH, HEIGHT, "Fuck Vulkan", nullptr, nullptr);
...
void HelloTriangleApplication::createSwapChain() {
	...
	if (vkCreateSwapchainKHR(device, &createInfo, nullptr, &swapChain) != VK_SUCCESS) {
		VEngine_Log("failed to create swap chain!");
		VENGINE_EXIT;
	}
...
void HelloTriangleApplication::createImageViews() {
	...
	for (auto i : vstd::range(swapChainImages.size())) {
		...
		if (vkCreateImageView(device, &createInfo, nullptr, &swapChainImageViews[i]) != VK_SUCCESS) {
			VEngine_Log("failed to create image views!");
			VENGINE_EXIT;
		}
	}
}
vstd::vector<vbyte> HelloTriangleApplication::readFile(const vstd::string& filename) {
	BinaryReader reader(filename);
	return reader.Read();
}

VkShaderModule HelloTriangleApplication::createShaderModule(const vstd::vector<vbyte>& code) {
	...
	if (vkCreateShaderModule(device, &createInfo, nullptr, &shaderModule) != VK_SUCCESS) {
		VEngine_Log("failed to create shader module!");
		VENGINE_EXIT;
	}
	return shaderModule;
}
void HelloTriangleApplication::createGraphicsPipeline() {
	...
	if (vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &pipelineLayout) != VK_SUCCESS) {
		VEngine_Log("failed to create pipeline layout!");
		VENGINE_EXIT;
	}
	...
	if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline) != VK_SUCCESS) {
		VEngine_Log("failed to create graphics pipeline!");
		VENGINE_EXIT;
	}
}
void HelloTriangleApplication::createRenderPass() {
	...
	if (vkCreateRenderPass(device, &renderPassInfo, nullptr, &renderPass) != VK_SUCCESS) {
		VEngine_Log("failed to create render pass!");
		VENGINE_EXIT;
	}
}
void HelloTriangleApplication::createFrameBuffers() {
	swapChainFramebuffers.resize(swapChainImageViews.size());
	for (auto i : vstd::range(swapChainImageViews.size())) {
		...
		if (vkCreateFramebuffer(device, &framebufferInfo, nullptr, &swapChainFramebuffers[i]) != VK_SUCCESS) {
			VEngine_Log("failed to create framebuffer!");
			VENGINE_EXIT;
		}
	}
}
void HelloTriangleApplication::createCommandPool() {
	...
	if (vkCreateCommandPool(device, &poolInfo, nullptr, &commandPool) != VK_SUCCESS) {
		VEngine_Log("failed to create command pool!");
		VENGINE_EXIT;
	}
}
void HelloTriangleApplication::createCommandBuffer() {
	...
	allocInfo.level = VK_COMMAND_BUFpdFER_LEVEL_PRIMARY;
	allocInfo.commandBufferCount = 1;

	if (vkAllocateCommandBuffers(device, &allocInfo, &commandBuffer) != VK_SUCCESS) {
		VEngine_Log("failed to allocate command buffers!");
		VENGINE_EXIT;
	}
}
void HelloTriangleApplication::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
	// Begin
	{
		...
		if (vkBeginCommandBuffer(commandBuffer, &beginInfo) != VK_SUCCESS) {
			VEngine_Log("failed to begin recording command buffer!");
			VENGINE_EXIT;
		}
	}
	// Begin Pass
	...
	}
	{
		if (vkEndCommandBuffer(commandBuffer) != VK_SUCCESS) {
			VEngine_Log("failed to record command buffer!");
			VENGINE_EXIT;
		}
	}
}
...
int main() {
	...
	toolhub::vk::HelloTriangleApplication app;
	app.Run();
	return EXIT_SUCCESS;
}